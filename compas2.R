...
# new
f <- as.numeric(df$crime_factor) - 1
...
# new
# Prior for coefficient of U on crime_factor
mu_uf <- 0
sigma_uf <- 1

# Prior for intercept on equation for crime_factor
mu_f0 <- 0
sigma_f0 <- 1
...
compas_stan_dat <- list(N = n, K = p, x = x, s = s, y = y, f = f, # new 
                        mu_eta = mu_eta, sigma_eta = sigma_eta,
                        mu_us = mu_us, sigma_us = sigma_us, 
                        mu_uy = mu_uy, sigma_uy = sigma_uy, 
                        mu_y0 = mu_y0, sigma_y0 = sigma_y0,
                        mu_uf = mu_uf, sigma_uf = sigma_uf, # new
                        mu_f0 = mu_f0, sigma_f0 = sigma_f0) # new

#fit <- stan(file = 'compas.stan', data = compas_stan_dat, iter = 1000, chains = 1, verbose = TRUE)
fit <- stan(file = 'compas2.stan', data = compas_stan_dat, iter = 1000, chains = 1, verbose = TRUE)
...
output <- data.frame(two_year_recid = df$two_year_recid,
                     ...
                     crime_factor = df$crime_factor, # new
                     s = s, race = race, age = age,
                     gender = gender, u_hat = u_hat)

write.csv(output, file = "compas2_stan_results.csv", row.names = FALSE)
