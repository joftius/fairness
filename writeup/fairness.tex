...
obeys the FTA criterion but is not counterfactually fair, so omitting
$A$ (race blindness) may \emph{introduce} unfairness into an otherwise
fair world.
%
...
%%%%% END ICML STUFF
A city government wants to know the yearly crime rate $Y$ in each
neighborhood $X$. Racial demographics are
also available, and for simplicity suppose these are encoded as a binary
$A$ indicating a minority-majority neighborhood.
The past data on crime rates depends on differential rates of policing
which disproportionately affect minority neighborhoods.
This can be seen in Figure~\ref{figure.simple_models}(d)
(\emph{Right}). Unlike Scenario 1, $Y$ now
...
The lesson from the last example is that in general we need a
