...
random variable that depends on $A, X$ and $U$, and which is given by
a machine learning algorithm as a prediction for $Y$.
...
instance historically biased distributions, where individuals
with different protected attributes $A$ may have many different
attributes $X$ such as their current level of wealth. Disparities may be due to
discriminative measures in hiring practices and perpetuated by lacking
of educational opportunities in the typical geographical location of
minorities.
...
\begin{define}[Fairness Through Unawareness (FTU)]
  ...
  $\hat{Y}: X \rightarrow Y$ that excludes $A$ satisfies this.
...
approach has a compelling simplicity, but it has a clear shortcoming
as elements of $X$ can contain discriminatory information analogous to $A$ that may
not be obvious only through the assessment of their association. The need
for expert knowledge in assessing the relationship between
$A$ and $X$ was highlighted in the work on individual fairness:
...
%\end{align}
\end{define}
These criteria can be incompatible in
general, as discussed by \citet{kleinberg2016inherent} and \citet{berk:17}.
Following the motivation of IF, we propose that knowledge
about individual differences should be taken into consideration,
even if strong assumptions are necessary. Moreover, it is not clear
for any of these approaches in which ways historical biases can be
...
\item $U$ is a set of latent {\bf background} variables,%\footnote{These are
  ...
  which factors not caused by any variable in the set $V$ of observable variables;
